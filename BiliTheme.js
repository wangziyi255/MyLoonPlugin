/*
* 📺 BiliBili: 👘 Skin Argument Loader (防呆修复版)
* 核心逻辑：读取 Loon 插件参数，注入皮肤数据
*/
...
/***************** Processing *****************/
(async () => {
    // 🛡️ 防呆检测：如果你手动点击运行，这里会拦截并提示，不再报错
    if (typeof $request === "undefined") {
        $.msg("📺 BiliBili 换肤", "⚠️ 无法手动运行", "请直接打开 B 站 App，脚本会自动触发。");
        $.log("⚠️ 错误：脚本未在网络请求中运行。$request 未定义。请勿直接点击运行。");
        $.done();
        return;
    }
    ...
    if (PATH.includes("x/resource/show/skin") && RAW_ARGUMENT) {
        try {
            ...
            $.log(`🚫 注入失败: ${e.message}`);
            // 如果 JSON 格式还是有问题，这里会打出日志，记得去日志看
        }
    } else {
        // 这一步是给调试看的，证明脚本确实被 B 站唤醒了，只是没轮到皮肤接口
        // $.log(`🔍 跳过接口: ${PATH}`); 
    }
